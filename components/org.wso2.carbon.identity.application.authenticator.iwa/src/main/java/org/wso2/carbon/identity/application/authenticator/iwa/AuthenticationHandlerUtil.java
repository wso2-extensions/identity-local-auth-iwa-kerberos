...
public class AuthenticationHandlerUtil {
   ...
   //     serverPrincipal = new KerberosPrincipal(getPrincipalNameWithoutDomain(serverCred.getName().toString()));
    }

    private static String getPrincipalNameWithoutDomain(String fullQualifiedDomainName) {
        String principalName = fullQualifiedDomainName;

        if (principalName.contains("@")) {
            principalName = fullQualifiedDomainName.split("@")[0];
        }
    ...
    public static void setKeberosConfigFilePath() {
        ...
        if (StringUtils.isEmpty(kerberosFilePath)) {
            ...
            kerberosFilePath = Paths.get(carbonHome,"repository", "conf", "identity", "krbf.conf").toString();
